<template>
  <div class="flex justify-center items-center gap-2 border-x px-2 border-base-300">
    <button
      class="pagination-btn hidden sm:block"
    ...
    <template v-for="pageNumber in displayedPages" :key="pageNumber">
      <button class="pagination-btn hidden sm:block" :class="{ 'active-pagination-btn': pageNumber === currentPage }" @click="goToPage(pageNumber)">
        {{ pageNumber }}
      </button>
    </template>
    <button
